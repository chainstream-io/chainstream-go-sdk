/*
Dex Aggregator API

Dex Aggregator API ⚡️

API version: 1.0
Contact: ai@sx.ai
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the SwapRouteResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SwapRouteResponse{}

// SwapRouteResponse struct for SwapRouteResponse
type SwapRouteResponse struct {
	// Original swap request parameters
	Args SwapRouteInput `json:"args"`
	// Base64 encoded transaction
	SerializedTx string `json:"serializedTx"`
	// Detailed routing information
	RouteInfo map[string]interface{} `json:"routeInfo"`
	// Time taken to process the request in milliseconds
	ElapsedTime int64 `json:"elapsedTime"`
}

type _SwapRouteResponse SwapRouteResponse

// NewSwapRouteResponse instantiates a new SwapRouteResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSwapRouteResponse(args SwapRouteInput, serializedTx string, routeInfo map[string]interface{}, elapsedTime int64) *SwapRouteResponse {
	this := SwapRouteResponse{}
	this.Args = args
	this.SerializedTx = serializedTx
	this.RouteInfo = routeInfo
	this.ElapsedTime = elapsedTime
	return &this
}

// NewSwapRouteResponseWithDefaults instantiates a new SwapRouteResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSwapRouteResponseWithDefaults() *SwapRouteResponse {
	this := SwapRouteResponse{}
	return &this
}

// GetArgs returns the Args field value
func (o *SwapRouteResponse) GetArgs() SwapRouteInput {
	if o == nil {
		var ret SwapRouteInput
		return ret
	}

	return o.Args
}

// GetArgsOk returns a tuple with the Args field value
// and a boolean to check if the value has been set.
func (o *SwapRouteResponse) GetArgsOk() (*SwapRouteInput, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Args, true
}

// SetArgs sets field value
func (o *SwapRouteResponse) SetArgs(v SwapRouteInput) {
	o.Args = v
}

// GetSerializedTx returns the SerializedTx field value
func (o *SwapRouteResponse) GetSerializedTx() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.SerializedTx
}

// GetSerializedTxOk returns a tuple with the SerializedTx field value
// and a boolean to check if the value has been set.
func (o *SwapRouteResponse) GetSerializedTxOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SerializedTx, true
}

// SetSerializedTx sets field value
func (o *SwapRouteResponse) SetSerializedTx(v string) {
	o.SerializedTx = v
}

// GetRouteInfo returns the RouteInfo field value
func (o *SwapRouteResponse) GetRouteInfo() map[string]interface{} {
	if o == nil {
		var ret map[string]interface{}
		return ret
	}

	return o.RouteInfo
}

// GetRouteInfoOk returns a tuple with the RouteInfo field value
// and a boolean to check if the value has been set.
func (o *SwapRouteResponse) GetRouteInfoOk() (map[string]interface{}, bool) {
	if o == nil {
		return map[string]interface{}{}, false
	}
	return o.RouteInfo, true
}

// SetRouteInfo sets field value
func (o *SwapRouteResponse) SetRouteInfo(v map[string]interface{}) {
	o.RouteInfo = v
}

// GetElapsedTime returns the ElapsedTime field value
func (o *SwapRouteResponse) GetElapsedTime() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.ElapsedTime
}

// GetElapsedTimeOk returns a tuple with the ElapsedTime field value
// and a boolean to check if the value has been set.
func (o *SwapRouteResponse) GetElapsedTimeOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ElapsedTime, true
}

// SetElapsedTime sets field value
func (o *SwapRouteResponse) SetElapsedTime(v int64) {
	o.ElapsedTime = v
}

func (o SwapRouteResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SwapRouteResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["args"] = o.Args
	toSerialize["serializedTx"] = o.SerializedTx
	toSerialize["routeInfo"] = o.RouteInfo
	toSerialize["elapsedTime"] = o.ElapsedTime
	return toSerialize, nil
}

func (o *SwapRouteResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"args",
		"serializedTx",
		"routeInfo",
		"elapsedTime",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varSwapRouteResponse := _SwapRouteResponse{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varSwapRouteResponse)

	if err != nil {
		return err
	}

	*o = SwapRouteResponse(varSwapRouteResponse)

	return err
}

type NullableSwapRouteResponse struct {
	value *SwapRouteResponse
	isSet bool
}

func (v NullableSwapRouteResponse) Get() *SwapRouteResponse {
	return v.value
}

func (v *NullableSwapRouteResponse) Set(val *SwapRouteResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableSwapRouteResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableSwapRouteResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSwapRouteResponse(val *SwapRouteResponse) *NullableSwapRouteResponse {
	return &NullableSwapRouteResponse{value: val, isSet: true}
}

func (v NullableSwapRouteResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSwapRouteResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

